...
# Entity types worth surfacing for a research-paper corpus. spaCy's default
# labels (trained on general news text) still pick up a useful subset for
# abstracts: organizations, methods/products framed as named things, works,
# quantities/metrics, and locations tied to datasets or institutions.
RELEVANT_ENTITY_LABELS = {
    "ORG": "Organization",
    "PRODUCT": "Product/Model",
    "GPE": "Location",
    "PERSON": "Person",
    "WORK_OF_ART": "Named Work",
    "LAW": "Law/Standard",
    "PERCENT": "Percent",
...
def _load_ner_model(model_name: str = NER_MODEL_NAME):
    """Load the spaCy NER pipeline, downloading the small English model
    on first use if it isn't already installed."""
    try:
        return spacy.load(model_name)
    except OSError:
        print(f"spaCy model '{model_name}' not found locally — downloading it once...")
...
class PaperSearchEngine:
    ...
    def __init__(self, load_summarizer: bool = True, load_keybert: bool = True,
                 load_ner: bool = True, device: int = -1):
        ...
        if load_ner:
            ...
            self.ner_model = _load_ner_model(NER_MODEL_NAME)
    ...
    def extract_entities(self, text: str) -> list[dict]:
        """
        Run NER over an abstract and return the named entities spaCy finds
        (organizations, people, locations, dates, metrics, etc.), filtered
        down to the label set that's actually useful for a research-paper
        corpus and de-duplicated by (text, label).

        Returns a list of {"text": ..., "label": ..., "label_desc": ...}.
        """
        if self.ner_model is None:
            raise RuntimeError("NER model not loaded. Init with load_ner=True.")

        doc = self.ner_model(text[:3000])  # defensive cap, mirrors summarize()
        seen = set()
        entities = []
        for ent in doc.ents:
            ...
            entities.append({
                "text": ent.text.strip(),
                "label": ent.label_,
                "label_desc": RELEVANT_ENTITY_LABELS[ent.label_],
            })
        return entities

    # ------------------------------------------------------------------ #
    # Convenience: search + summarize + keywords + entities in one call
    # ------------------------------------------------------------------ #
    def full_report(self, query: str, k: int = 5) -> list[dict]:
        """
        Run a search and enrich every hit with an AI-generated summary,
        extracted keywords, and named entities -- this is what powers the
        Streamlit demo.
        """
