...
with st.sidebar:
    ...
    show_entities = st.checkbox("Detect named entities (NER)", value=True)
    st.markdown("---")
    st.markdown(
        "**How it works**\n\n"
        ...
        "5. spaCy NER tags organizations, models, dates & metrics"
    )
...
if search_clicked and query.strip():
    ...
    with st.spinner("Searching and analyzing papers..."):
        ...
        for r in results:
            ...
            if show_entities:
                r["entities"] = engine.extract_entities(r["abstract"])
    ...
    for r in results:
        with st.container(border=True):
            ...
            if show_entities and "entities" in r:
                if r["entities"]:
                    ent_tags = "  ".join(
                        f"`{ent['text']}` _{ent['label_desc']}_" for ent in r["entities"]
                    )
                    st.markdown(f"**🧬 Named Entities:** {ent_tags}")
                else:
                    st.markdown("**🧬 Named Entities:** _none detected_")
