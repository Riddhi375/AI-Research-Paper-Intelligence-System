...
5. **Tags named entities** — detects organizations, models/products, dates, locations, and metrics mentioned in the abstract using spaCy NER
...
```
        ...
        user query ──encode──────┘
           ...
           ▼                     ▼                     ▼
┌────────────────────┐ ┌────────────────────┐ ┌────────────────────┐
│   BART Summarizer   │ │   KeyBERT Keywords  │ │    spaCy NER        │
│ (distilbart-cnn-12) │ │  (n-gram phrases)   │ │ (en_core_web_sm)    │
└────────────────────┘ └────────────────────┘ └────────────────────┘
           │                     │                     │
           └─────────────────────┼─────────────────────┘
                                 ▼
...
> The `en_core_web_sm` spaCy model is installed directly from `requirements.txt`.
> If you ever see an error that it's missing, `search_engine.py` will
> automatically fetch it on first run — or you can install it manually with
...
for r in results:
    ...
    print(r["summary"])
    print(r["keywords"])
    print(r["entities"])  # [{"text": "BERT", "label": "PRODUCT", "label_desc": "Product/Model"}, ...]
```
...
- **Why cache embeddings/index to disk?** Re-encoding 50,000 papers takes
  ...
  subsequent run.
- **Why spaCy for NER instead of a transformer NER model?** `en_core_web_sm`
  is lightweight (~13MB), loads instantly, and runs fast enough on CPU to tag
  entities for every search result in real time — no noticeable added latency
  to the search flow, unlike a full transformer-based NER pipeline.
...
- Fine-tune the NER model on a scientific corpus (e.g. SciERC) to tag
  domain-specific entities like datasets, metrics, and model architectures
  more precisely than the general-purpose `en_core_web_sm` labels allow
